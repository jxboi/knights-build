...
- Continue locally with autosaves, manual save, named villages, conflict-safe multi-tab behavior, and validated download/restore backups.
...
The palette starts with the six beginner tools — Cottage, Grain field,
Farmhouse, Lumberyard, Stone mine, and Path. Select `More buildings` to
reveal the deeper catalog. Number shortcuts still select the first nine
catalog entries, while `B` is a quick shortcut for Cottage:

| Key | Building | Cost | Effect |
| --- | --- | --- | --- |
| 1 | Watchtower | 45 wood · 20 stone | Expands the buildable boundary by 3 tiles |
| 2 | Cottage | 30 wood · 10 stone | +2 housing capacity; villagers are trained at the School |
| 3 | Well | 15 wood · 25 stone | Village gathering place |
| 4 | Grain field | 1 food per tile | Grows in 30 seconds; a farmhouse harvests 8 wheat per plot |
| 5 | Farmhouse | 25 wood · 5 stone | Farmers harvest connected ripe fields |
| 6 | Bakery | 40 wood · 25 stone | Turns 3 wheat into 5 bread per 12-second cycle; holds 5 |
| 7 | Inn | 55 wood · 25 stone | Holds 8 bread for 3 hungry villagers; overflow goes to a Storehouse |
| 8 | Storehouse | 40 wood · 60 stone | Adds 150 storage for every resource and posts 3 carriers |
| 9 | Lumberyard | 20 wood · 10 stone | Workers chop nearby trees and saw +8 wooden planks per cycle |
| — | Stone mine | 35 wood · 15 stone | +6 stone per cycle; holds 12 |
...
- Saves automatically in this browser, plus a manual Save village action in the top-right menu. Download or restore a JSON backup from the same menu. Sparse or partially repaired saves retain recoverable progress while keeping first-build milestones tied to completed player-built structures.
...
The top-left goals panel starts with one next step and keeps the full starter checklist behind `Milestones`; after those goals are complete it reveals the next chapter. The village overview adds a compact readout of population, completed structures, active jobs, deliveries, paths, worksite focus, worker roles, recent activity, graphics presets, atmosphere settings, and milestone progress. Select a listed worksite or villager to focus it in the world.

Use the village menu to rename the settlement, save immediately, download or restore a validated JSON backup, open the overview or help dialog, and reset to the original settlement. Saves remain in this browser under `hearth-v1`; there is no account-backed or cloud save. If another tab changes the same village, the older tab pauses and asks to reload before it can write again.
